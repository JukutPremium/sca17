...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_category'])) {
    ...
    if ($id) {
        ...
        $stmt = $conn->prepare("UPDATE categories SET name = ?, description = ?, icon = ?, is_active = ? WHERE id = ?");
        ...
        // Add new category
        $stmt = $conn->prepare("INSERT INTO categories (name, description, icon, is_active) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("sssi", $name, $description, $icon, $is_active);
        ...
        <?php while ($row = $result->fetch_assoc()): ?>
            <div class="bg-white rounded-lg shadow p-6 <?= $row['is_active'] ? '' : 'opacity-50' ?>">
                <div class="flex justify-between items-start mb-3">
                    <div class="text-4xl"><?= $row['icon'] ?></div>
        ...
        <form method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="icon" id="icon" maxlength="10"
                       placeholder="📂 (Copy paste emoji)"
                       class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                <p class="text-xs text-gray-500 mt-1">
                    Gunakan emoji untuk icon. Contoh: 🏫 🚽 🔬 📚 ⚽
                </p>
            </div>
